...
# data training and testing splitting of data
...
# Label for the title
label = tk.Label(root, text="NETWORK INTRUSION CLASSIFICATION SYSTEM",
...
def preprocess():
    ...
    attack_class_freq_train = data[['attack_class']].apply(
        lambda x: x.value_counts())
    attack_class_freq_test = data[['attack_class']].apply(
        lambda x: x.value_counts())
...
def plotAttackClass():
    text.insert(
        tk.END, "**************************************Plotting********************************\n\n\n")
    attack_class_plot = df[['frequency_percent_train',
...
def trainDecisionTree():
    ...
    # traning the model .fit() , .predict()--> predition, confusion_matrix
    ...
    text.insert(
        tk.END, "**************************************Decision Tree Model Evaluation********************************\n\n\n")
...
def compareGraph():
    ...
    # os.system("Chart.py")
    subprocess.Popen(
        ["python", "C:/Users/Rakes/OneDrive/Desktop/Network Intrusion Classification/Chart.py"])
...
def compareModels():
    ...
    # Train the CNN model
    cnn_model.fit(X_train.values.reshape(-1,
                  X_train.shape[1], 1), y_train, epochs=5, batch_size=32, verbose=1)

    # Make predictions on the testing set using the CNN model
    y_pred_cnn = cnn_model.predict_classes(
...
# -----------------------------BUILD BY ANONYMOUS-----------------------------#
    ...
    plt.bar(x_pos + bar_width, precisions, width=bar_width,
            label='Precision', color='green')
    plt.bar(x_pos + 2 * bar_width, recalls,
            width=bar_width, label='Recall', color='orange')
    plt.bar(x_pos + 3 * bar_width, f1_scores,
            width=bar_width, label='F1-Score', color='green')
...
# -----------------------------BUILD BY ANONYMOUS-----------------------------#
    ...
    for i, v in enumerate(accuracies):
        plt.text(i, v + 0.01, f'Acc: {round(v, 2)}',
                 ha='center', color='black', fontweight='bold')
        plt.text(i, precisions[i] + 0.01, f'Prec: {round(precisions[i], 2)}',
                 ha='center', color='black', fontweight='bold')
        plt.text(i, recalls[i] + 0.01, f'Rec: {round(recalls[i], 2)}',
                 ha='center', color='black', fontweight='bold')
        plt.text(i, f1_scores[i] + 0.01, f'F1: {round(f1_scores[i], 2)}',
                 ha='center', color='black', fontweight='bold')
...
button1 = tk.Button(frame, text="Upload Dataset",
                    command=uploadDataset, font=("Arial", 12))
button1.pack(side="left", padx=10)

button2 = tk.Button(frame, text="Preprocess",
                    command=preprocess, font=("Arial", 12))
button2.pack(side="left", padx=10)

button3 = tk.Button(frame, text="Plot Attack Class",
                    command=plotAttackClass, font=("Arial", 12))
button3.pack(side="left", padx=10)

button4 = tk.Button(frame, text="Train Decission Tree",
                    command=trainDecisionTree, font=("Arial", 12))
button4.pack(side="left", padx=10)

button5 = tk.Button(frame, text="Comparision Graph",
                    command=compareGraph, font=("Arial", 12))
